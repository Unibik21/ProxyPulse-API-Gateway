...
export async function recordCacheEvent(entry: LogEntry){
    ...
    if(entry.cacheStatus=== 'N/A'){
        await redis.hincrby(
            `analytics:${entry.orgId}:${entry.projectId}:cache_uncached`,
            entry.path,
            1
        );
        return;
    ...
    await redis.hincrby(`analytics:${entry.orgId}:${entry.projectId}:cache_status:${entry.path}`,field,1);
}
...
export async function computeCacheRecommendations(orgId: string, projectId: string): Promise<CacheRecommendation[]>{
    const recommendations: CacheRecommendation[]= [];

    const uncached = await redis.hgetall(`analytics:${orgId}:${projectId}:cache_uncached`);
    ...
    const prefix = `analytics:${orgId}:${projectId}:cache_status:`;
    const statKeys = await redis.keys(`${prefix}*`);
